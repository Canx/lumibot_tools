class Sizing:
    ...
    def __init__(self, strategy, min_cash, assets=None):
    ...
    def shares_to_buy(self, symbol, price_per_share, method='basic', **kwargs):
        if method == 'basic':
            return self.shares_to_buy_basic(symbol, price_per_share, self.cash, **kwargs)
        elif method == 'var':
            return self.shares_to_buy_var(symbol, price_per_share, **kwargs)
        else:
    ...
    def shares_to_buy_trend(self, asset, price_per_share, trend_strength_function, max_percentage_per_trade=1.0, ):
        if price_per_share <= 0:
            return 0.0
        ...
        available_cash = self.cash * max_percentage_per_trade * trend_strength_function(asset)
        
        if self.assets_type == "crypto":
            # Calcula y redondea la cantidad de criptomoneda a comprar según el máximo de decimales permitido
            shares_to_buy = available_cash / price_per_share
            shares_to_buy = round(shares_to_buy, self.max_crypto_decimals)
        elif self.assets_type == "stock":
            # Para acciones, redondea al número entero más cercano
